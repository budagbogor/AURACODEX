...
- [SANGAT FATAL] Jika Anda membuat \`postcss.config.js\`, gunakan plugin \`"@tailwindcss/postcss"\` dan \`autoprefixer\`. JANGAN menggunakan \`require('tailwindcss')\`, \`from "tailwindcss"\`, atau string plugin \`"tailwindcss"\` karena itu dapat memicu error ENOENT pada project hasil generate.
...
- [SANGAT FATAL] Jangan gunakan alias warna generik yang tidak ada di Tailwind seperti \`text-dark\`, \`bg-dark\`, \`text-light\`, \`bg-light\`, \`border-dark\`, atau \`border-light\`. Ganti dengan utility nyata seperti \`text-slate-900\`, \`bg-slate-900\`, \`text-slate-50\`, atau \`border-slate-100\`.
- [SANGAT FATAL] Jangan gunakan helper class semu seperti \`btn\`, \`btn-primary\`, \`btn-secondary\`, \`btn-outline\`, atau \`button\` di dalam \`@apply\`. Itu bukan utility Tailwind bawaan. Tulis langsung utility Tailwind yang lengkap pada elemen atau buat selector CSS biasa tanpa \`@apply btn\`.
